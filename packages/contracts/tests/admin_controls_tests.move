...
module holdemgame::admin_controls_tests {
    ...
    fun test_update_blinds_success(admin: &signer) {
        ...
        texas_holdem::update_blinds(admin, admin_addr, 10, 20);
        
        let (small, big, _, _) = texas_holdem::get_table_config(admin_addr);
        assert!(small == 10, 1);
        assert!(big == 20, 2);
    }
    ...
    fun test_update_blinds_non_admin_fails(admin: &signer, other: &signer) {
        ...
        texas_holdem::update_blinds(other, signer::address_of(admin), 10, 20);
    }
    ...
    fun test_update_buyin_limits_success(admin: &signer) {
        ...
        texas_holdem::update_buy_in_limits(admin, admin_addr, 100, 2000);
        
        let (_, _, min_buy, max_buy) = texas_holdem::get_table_config(admin_addr);
    ...
    fun test_update_buyin_non_admin_fails(admin: &signer, other: &signer) {
        ...
        texas_holdem::update_buy_in_limits(other, signer::address_of(admin), 100, 2000);
    }
    ...
    fun test_update_ante_success(admin: &signer) {
        ...
        texas_holdem::update_ante(admin, admin_addr, 2);
    ...
    fun test_toggle_straddle_success(admin: &signer) {
        ...
        // Enable straddle
        texas_holdem::toggle_straddle(admin, admin_addr, true);
        // Disable straddle
        texas_holdem::toggle_straddle(admin, admin_addr, false);
    }
    ...
    fun test_transfer_ownership_success(admin: &signer, new_admin: &signer) {
        ...
        texas_holdem::update_blinds(new_admin, admin_addr, 10, 20);
    ...
    fun test_old_admin_cannot_update_after_transfer(admin: &signer, new_admin: &signer) {
        ...
        texas_holdem::update_blinds(admin, admin_addr, 15, 30);
    }
    ...
    fun test_pause_resume_table_success(admin: &signer) {
        ...
        assert!(texas_holdem::is_paused(admin_addr) == true, 1);
        
        // Resume
        texas_holdem::resume_table(admin, admin_addr);
        assert!(texas_holdem::is_paused(admin_addr) == false, 2);
    }
    ...
    fun test_toggle_admin_only_start(admin: &signer) {
        ...
        texas_holdem::toggle_admin_only_start(admin, admin_addr, true);
        assert!(texas_holdem::is_admin_only_start(admin_addr) == true, 1);
        
        // Disable
        texas_holdem::toggle_admin_only_start(admin, admin_addr, false);
        assert!(texas_holdem::is_admin_only_start(admin_addr) == false, 2);
    }
    ...
    fun test_kick_player_success(admin: &signer, player: &signer) {
        ...
        // Kick player
        texas_holdem::kick_player(admin, admin_addr, 0);
    ...
    fun test_kick_player_non_admin_fails(admin: &signer, player: &signer, other: &signer) {
        ...
        texas_holdem::kick_player(other, admin_addr, 0);
    }
    ...
    fun test_force_sit_out_success(admin: &signer, player: &signer) {
        ...
        // Force sit out
        texas_holdem::force_sit_out(admin, admin_addr, 0);
